...
if [ -f "/etc/udev/rules.d/97-r2mini-lidar.rules" ]; then
    ...
    echo 'SUBSYSTEM=="tty", KERNELS=="1-2", MODE:="0666", GROUP:="dialout", SYMLINK+="ttyLIDAR"' > /etc/udev/rules.d/97-r2mini-lidar.rules
...
if [ -f "/etc/udev/rules.d/98-r2mini-mcu.rules" ]; then
    ...
    echo 'SUBSYSTEM=="tty", KERNELS=="1-3", MODE:="0666", GROUP:="dialout", SYMLINK+="ttyMCU"' > /etc/udev/rules.d/98-r2mini-mcu.rules
